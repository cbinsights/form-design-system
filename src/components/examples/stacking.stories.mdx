import { Meta, Story, Preview, Props } from "@storybook/addon-docs/blocks";
import { useRef, useState } from 'react'
import {
  withKnobs,
  optionsKnob as options,
  select,
  number,
  boolean,
} from '@storybook/addon-knobs';
import { arrayToOptions, Info, ImportPath } from 'components/util/storybook';

import Popover from 'components/popovers/Popover';
import Button from 'components/interactive/Button';
import Dialog from 'components/modals/Dialog';
import Menu from 'components/popovers/Menu';
import MenuItem from 'components/popovers/MenuItem';

<Meta title="Examples/Stacking" />

## Show stacking interactions

export const MenuExample = () => (
  <Menu trigger={<Button label="Menu trigger" hasCaret />}>
    <MenuItem>
      Copy
    </MenuItem>
    <MenuItem>
      Delete
    </MenuItem>
  </Menu>
)

<Preview>
  <Story name="Stacking">
    {() => {
      const Stacking = () => {
        const [isDialogShowing, setIsDialogShowing] = useState();
        const showDialog = () => setIsDialogShowing(true);
        const hideDialog = () => setIsDialogShowing(false);
        const PopoverExample = () => (
            <Popover
                trigger={<button>Open popover</button>}
                position="bottom"
                alignment="start"
                distance={8}
            >
                <div className="bgColor--white rounded--all elevation--2 padding--all">
                    <Stacking />
                </div>
            </Popover>
        )
        return (
            <>
            <button onClick={showDialog}>Open Dialog</button>
            <MenuExample />
            <PopoverExample />
            <Dialog
                isOpen={isDialogShowing}
                content={<Stacking />}
                onDismiss={hideDialog}
            />
            </>
        )
      } 
      const [isScrolling, setIsScrolling] = useState(true)
      const toggleScroll = () => setIsScrolling(!isScrolling);
      const [isFixed, setIsFixed] = useState(true)
      const toggleFixed = () => setIsFixed(!isFixed);
      return (
        <div style={{ height: isScrolling ? "3000px" : undefined }}>
          <div style={{ position: isFixed ? "fixed" : undefined }}>
            <button onClick={toggleScroll}>{isScrolling ? 'Turn off scroll' : 'Turn on scroll'}</button>
            <button onClick={toggleFixed}>{isFixed ? 'Turn off fixed' : 'Turn on fixed'}</button>
            <Stacking />
          </div>
        </div>
      )
    }}
  </Story>
</Preview>