...
state of the popover is exclusively controlled by the prop `isOpen`.

To update the state of `isOpen` based on user interaction, you must use the `onUserDismiss` callback.
...
If you're not sure how to handle scrolling behavior with a popover, choose this option.
