import { Meta, Story, Preview, Props } from "@storybook/addon-docs/blocks";
import { action } from "@storybook/addon-actions";
import {
  withKnobs,
  text,
  boolean,
  optionsKnob as options
} from "@storybook/addon-knobs";
import ImportPath from 'util/storybook-docs/ImportPath';
import { arrayToOptions } from 'util/storybook-docs/knobHelpers';
import { StoryWrapper, StoryItem } from 'util/storybook-docs/StoryLayout';
import StarFilledIcon from "lib/icons/react/StarFilledIcon";
import CompanyIcon from 'lib/icons/react/CompanyIcon';

import Chip, { THEMES, SIZES } from ".";

export const chips = [
  { label: "Lorem", href: "#lorem" },
  { label: "Ipsum", href: "#ipsum", isActive: true, subtitle: "active" },
  { label: "Dolor", href: "#dolor", onClose: () => {} },
  {
    label: "Sit",
    onClick: () => action("custom chip clicked"),
    isActive: false
  }
];

export const chipsTwo = [
  { label: "Lorem Ipsum" },
  { label: "Dolor Sit" },
  { label: "Sit Amet" },
  { label: "Consectetur", onClick: () => action("custom chip clicked") }
];

<Meta title="Components/Chip" decorators={[withKnobs]} component={Chip} />

# Chip

<ImportPath component={Chip} />

<Story name="Knobs" parameters={{ docs: { disable: true }}}>
  <Chip
    isActive={boolean("isActive", false)}
    Icon={boolean("Icon", false) ? CompanyIcon : undefined}
    label={text("label", "Hello World")}
    subtitle={text("subtitle", "")}
    onClose={boolean("onClose", true) ? action("closed") : undefined}
    onClick={action("clicked")}
    size={options("size", arrayToOptions(SIZES), undefined, {
      display: "inline-radio"
    })}
    theme={options("theme", THEMES, undefined, { display: "inline-radio" })}
  />
</Story>

<Preview isExpanded>
  <Chip
    label="Hello world"
  />
</Preview>

<Props of={Chip} />

Used to display tags, members, filter settings, and other tokens.

### Themes / States

<Preview>
  <Story name="Themes">
    <div>
    {THEMES.map(theme => (
      <div className="margin--right--half margin--bottom--half display--inlineBlock">
        <Chip theme={theme} label={theme} />
      </div>
    ))}
    <br />
    {THEMES.map(theme => (
      <div className="margin--right--half margin--bottom--half display--inlineBlock">
        <Chip theme={theme} label={theme} isActive />
      </div>
    ))}
    </div>
  </Story>
</Preview>

### Sizes

<Preview>
  <Story name="Sizes">
    {SIZES.map(size => (
      <div className="margin--right--half margin--bottom--half display--inlineBlock">
        <Chip size={size} label={`Size: ${size}`} />
      </div>
    ))}
  </Story>
</Preview>

### Subtitle

<Preview>
  <Story name="Subtitle">
    {SIZES.map(size => (
      <div className="margin--right--half margin--bottom--half display--inlineBlock">
        <Chip size={size} label="Chip" subtitle="subtitle" />
      </div>
    ))}
  </Story>
</Preview>

### Misc

<Preview>
  <Story name="Misc">
    <StoryWrapper>
      <StoryItem>
        <Chip label="#amet" theme="ghost" onClose={() => {}} />
        <Chip label="#sit" theme="ghost" onClose={() => {}} />
        <Chip label="#dolor" theme="ghost" onClose={() => {}} />
        <Chip label="#ipsum" theme="ghost" onClose={() => {}} Icon={StarFilledIcon} />
      </StoryItem>
    </StoryWrapper>
  </Story>
</Preview>