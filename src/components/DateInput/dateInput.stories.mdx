...
<ImportPath component={DateInput} />
...
Pass an instance of `Date` or a parsable date string to the `defaultDate` prop to set a default value.
Due to inconsistency across browsers with how native `Date` parsing works, we recommend
passing a string to `defaultDate`.
