  ...
  render={(size, setSize) => (
    <section>
        ...
        <Flex>
          ...
          <FlexItem align="center" shrink>
            <div className="border--left padding--left">
              <label className="type--data flush--bottom display--inlineBlock padding--right--s" htmlFor="size">Size</label>
              <select
                name="size"
                value={size}
      ...
      <ul className="resetList iconList">
        {manifest.map(({name, svg}) => <Icon name={name} svg={svg} size={size} />)}
      </ul>
    </section>
  )}
/>
