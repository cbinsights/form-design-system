...
const { FDS_PACKAGE_NAME, getSourceFilePaths } = require('./searchUtils.js');
 ...
 */
const toAst = (path) =>
  parser.parse(fs.readFileSync(path).toString(), {
    sourceType: 'module',
...
getSourceFilePaths(process.argv[2], (files) => {
  const componentCountMap = files
    ...
    .reduce((acc, curr) => {
      // count up the number of imports for each component in an obj
      if (!acc[curr]) acc[curr] = 0;
      acc[curr]++;
      return acc;
    }, {});

  const totalImports = Object.values(componentCountMap).reduce(
    (acc, curr) => acc + curr,
    0
  );
