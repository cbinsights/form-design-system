module.exports = (file, api) => {
    ...
    root = root.find(j.JSXElement, {
      openingElement: { name: { name: componentName } }
    }).forEach((statement) => {
       const filteredAttributes = statement.value.openingElement.attributes.filter(obj => { 
          return !(obj.name.name === 'isInModal');
        })
          j(statement).replaceWith(
            j.jsxElement(
              j.jsxOpeningElement(j.jsxIdentifier(statement.value.openingElement.name.name), filteredAttributes, statement.value.openingElement.selfClosing),
              !statement.value.openingElement.selfClosing ? statement.value.closingElement : null,
